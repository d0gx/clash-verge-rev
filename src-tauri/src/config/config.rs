...
impl Config {
    ...
    pub async fn init_runtime_config() -> Result<()> {
        ...
        // Startup validation produced the exact runtime that the first core
        // process will consume. Commit it before service ownership/ICS
        // synchronization begins so background work never needs draft data.
    ...
    pub async fn generate_file(typ: ConfigType) -> Result<PathBuf> {
        ...
        // Keep the in-memory runtime as the user's durable intent. A
        // non-elevated Windows sidecar fallback is an execution concern only,
        // so apply it to the Run-file snapshot instead of poisoning later
        // profile generation or persistent service/ICS synchronization.
        let mut file_config = config.clone();
        #[cfg(target_os = "windows")]
        if matches!(typ, ConfigType::Run) {
            apply_windows_sidecar_tun_fallback(
                &mut file_config,
                CoreManager::global().windows_sidecar_tun_fallback_active(),
            );
        }

        help::save_yaml(&path, &file_config, Some("# Generated by Clash Verge")).await?;
        Ok(path)
    }
...
fn apply_windows_sidecar_tun_fallback(config: &mut Mapping, active: bool) {
    if !active {
        return;
    }

    if let Some(Value::Mapping(tun)) = config.get_mut("tun") {
        tun.insert(Value::from("enable"), Value::Bool(false));
    }
}
...
mod tests {
    ...
    #[test]
    fn windows_sidecar_fallback_changes_only_the_execution_snapshot() -> Result<()> {
        let mut config = serde_yaml_ng::from_str::<Mapping>("tun:\n  enable: true\n  device: Mihomo\nmode: rule\n")?;
        let durable = config.clone();

        apply_windows_sidecar_tun_fallback(&mut config, true);
    ...
    #[test]
    fn inactive_windows_sidecar_fallback_preserves_tun_intent() -> Result<()> {
        let mut config = serde_yaml_ng::from_str::<Mapping>("tun:\n  enable: true\n")?;
        let expected = config.clone();

        apply_windows_sidecar_tun_fallback(&mut config, false);
